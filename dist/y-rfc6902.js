(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global['y-rfc6902'] = {}, global.Y));
}(this, (function (exports, Y) { 'use strict';
  ...
  class YPointer {
      ...
      */
      static fromJSON(path) {
          const tokens = path.split('/').map(unescape$1);
          if (tokens[0] !== '')
  ...
  */
      ...
      switch (typeof obj) {
          ...
          case 'object':
              if (Array.isArray(obj)) {
                  const y = new Y__namespace.Array();
  ...
  @returns copy of source where every Array and Object have been recursively
           reconstructed from their constituent elements
  */
  function cloneYMaybe(source) {
      if (!(source instanceof Y__namespace.AbstractType)) {
          return clone(source);
      }
      if (source instanceof Y__namespace.Array) {
          const length = source.length;
          const arrayTarget = new Array(length);
          for (let i = 0; i < length; i++) {
      ...
      if (source instanceof Y__namespace.Map) {
          const entries = Array.from(source.entries());
          return new Y__namespace.Map(entries);
      }
      console.log('cannot serialize type', source);
      throw new Error('cannot serialize type');
  }
  function setOnYMaybe(target, value, key) {
      const yValue = objectToY(value);
      if (target instanceof Y__namespace.Map) {
          target.set(key, yValue);
      }
      else if (target instanceof Y__namespace.Array) {
  ...
  function arraySpliceYMaybe(target, index = 0, deletions = 0, insertions = []) {
  ...
  */
  ...
  function _add(object, key, value) {
      if (object instanceof Y__namespace.AbstractType) {
          if (object instanceof Y__namespace.Map) {
              object.set(key, value);
          }
          else if (object instanceof Y__namespace.Array) {
  ...
  function _remove(object, key) {
      if (Array.isArray(object) || object instanceof Y__namespace.Array) {
  ...
  */
  function add(object, operation, cloneValue = true) {
  ...
  */
  ...
  case to `object`.
  */
  function apply(object, operation, cloneValue) {
  ...
  */
  function applyPatch(object, patch, cloneValue) {
  ...
  */
  function createYTest(input, path) {
      const doc = new Y.Doc();
  ...
  */
  function createYTests(input, patch) {
  ...
  exports.createYTests = createYTests;
